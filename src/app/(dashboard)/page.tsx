  ...
  useEffect(() => {
    async function fetchStats() {
      try {
        const [
          { count: packagesCount },
          { count: activeDestinationsCount },
          { count: unresolvedInquiriesCount },
          { count: storiesCount },
        ] = await Promise.all([
        ...
        setStats({
          ...
          destinations: activeDestinationsCount || 0,
          inquiries: unresolvedInquiriesCount || 0,
  ...
  return (
    <div className="grid content-start px-5 py-10 gap-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 md:gap-8 lg:grid-cols-4">
        <Link href="/packages" className="grid">
        ...
        <Link href="/forms" className="grid">
          <Card className="bg-[#F5E8C7] border-none text-[#2D2D2D]">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">
                Unresolved Forms
